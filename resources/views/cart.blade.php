        ...
        @foreach(\Cart::getContent() as $item)
          <div class="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
            <div class="flex w-2/5"> <!-- product -->
              <div class="w-20">
                <img class="h-24" src="https://via.placeholder.com/176" alt="">
              </div>
              <div class="flex flex-col justify-between ml-4 flex-grow">
                <span class="font-bold text-sm">{{ Str::words($item->name,20) }}</span>
                <span class="text-red-500 text-xs">{{ Str::words($item->description,20) }}</span>
                <a href="{{ route('checkout.cart.remove', $item->id) }}" class="font-semibold hover:text-red-500 text-gray-500 text-xs">Remove</a>
              </div>
            </div>
